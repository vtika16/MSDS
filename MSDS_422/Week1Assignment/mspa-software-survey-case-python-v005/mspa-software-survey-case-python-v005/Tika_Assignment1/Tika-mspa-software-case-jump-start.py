...
# Let's compare each tool for personal, professional
# and insdustrial and let's break down how the distributions look within each
# category. First I will want to look at the mean of each category

personal = survey_df.filter(like='My', axis=1)
personal.columns = ['Java','JS','Python','R','SAS']

professional = survey_df.filter(like='Prof', axis=1)
professional.columns = ['Java','JS','Python','R','SAS']

industry = survey_df.filter(like='Ind', axis=1)
industry.columns = ['Java','JS','Python','R','SAS']


personalmean = personal.mean(axis=0)
professionalmean = professional.mean(axis=0)
industrymean = industry.mean(axis=0)


personalmean.plot(kind='bar', title = "Means of Personal software need")
plt.savefig("PersonalMeanBarPlot.pdf")

professionalmean.plot(kind='bar', title = "Means of Professional Software need")
plt.savefig("ProfessionalMeanBarPlot.pdf")

industrymean.plot(kind='bar', title = "Means of Industrial Software need")
plt.savefig("IndustryMeanBarPlot.pdf")


# From the results, it seems as though both R and Python are the two big
# contenders for the survey results. Let's take the distribution of each 
# category and compare how they change between the categories

# selecting columns related to R
r = survey_df.filter(like='_R', axis=1)
# renaming columns
r.columns = ['personal', 'professional', 'industry']

# selecting columns related to python
python = survey_df.filter(like='_P', axis=1)
# renaming columns
python.columns = ['personal', 'professional', 'industry']

# let's use seaborn's pairplot to plot the relationship between each
# pair of columns for the need of R

pairr = sns.pairplot(r)
plt.savefig('pairplotR.pdf')


# let's do something similar for Python
pairpython = sns.pairplot(python)
plt.savefig('pairplotPython.pdf')

# This is showing that the need for both R and Python through the surveyors'
# perspective are more for professional than personal and industrial. 

# now lets see the interests of courses from the surveyors
...
courses = courses(survey_df)

courses.mean().sort_values().plot(kind='bar', title = "Means of Course Interest")
